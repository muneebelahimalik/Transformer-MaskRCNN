...
from keras.layers import Input, Conv2D, MaxPooling2D, Flatten, Dense, concatenate, BatchNormalization, Dropout, Activation
from keras.models import Model
from keras import initializers

from tmrcnn import utils
...
def _normalize_depth_vars(depth_k, depth_v, filters):
    ...
    """
    #Using 64 filters, with depth_k and depth_v as 0.25, so normalizing to 16
...
def MultiHead_conv2d(ip, filters, kernel_size=(3, 3), strides=(1, 1),
    ...
    conv_out = KL.Conv2D(filters - depth_v, kernel_size, strides=strides)(ip)
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True,
                 depth_k=0.015625, depth_v=0.015625, num_heads=1, relative_encodings=True):
    """Build a ResNet graph.
        architecture: Can be resnet50 or resnet101
        stage5: Boolean. If False, stage5 of the network is not created
        train_bn: Boolean. Train or freeze Batch Norm layers
        depth_k: float or int. Number of filters for k in augmented attention blocks.
            If passed as float, computed as `filters * depth_k`.
        depth_v: float or int. Number of filters for v in augmented attention blocks.
            If passed as float, computed as `filters * depth_v`.
        num_heads: int. Number of attention heads in augmented attention blocks.
            Must be set such that `depth_k // num_heads` is > 0.
        relative_encodings: bool. Whether to use relative encodings in augmented attention blocks or not.
    """
...
class TMaskRCNN():
    ...
    def build(self, mode, config):
        ...
        print("Image Input Shape:", input_image)
        ...
        if callable(config.BACKBONE):
            _, C2, C3, C4, C5 = config.BACKBONE(input_image, stage5=True,
                                            ...
                                            depth_k=0.015625, depth_v=0.015625,
